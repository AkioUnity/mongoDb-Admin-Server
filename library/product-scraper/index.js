...
exports.sites = {
	...
	amazon: function(product_id){
		...
		productSelector.review = x(['#averageCustomerReviews > span > span > span > a > i > span']);
		productSelector.pen_name = x(['#bylineInfo']);
		// if(productSelector.review.search("Be the first to review this item") == -1){
		// 	productSelector.review = 'Nillll';
		// }else{
		// 	productSelector.review = 'OKKKK';
		// }
	...
	target: function(path){
		...
		productSelector.review = x(['#averageCustomerReviews']);
		productSelector.pen_name = x(['#bylineInfo']);
